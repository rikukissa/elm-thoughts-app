...
Elm.Counter.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var isEnter = function (code) {    return _U.eq(code,13) ? $Result.Ok({ctor: "_Tuple0"}) : $Result.Err("not the right key code");};
   ...
   var thoughItem = function (thought) {    return A2($Html.li,_U.list([]),_U.list([$Html.text(thought)]));};
   var thoughList = function (thoughs) {    return A2($Html.ul,_U.list([]),A2($List.map,thoughItem,thoughs));};
   var update = F2(function (action,model) {
      var _p1 = action;
      if (_p1.ctor === "UpdateField") {
            return _U.update(model,{text: _p1._0});
         } else {
            return _U.update(model,{text: "",thoughs: A2($Basics._op["++"],model.thoughs,_U.list([_p1._0]))});
         }
   });
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([thoughList(model.thoughs)
              ,$Html.text("Add thought")
              ,A2($Html.input,
              _U.list([$Html$Attributes.$class("form-control")
                      ,$Html$Attributes.type$("text")
                      ,$Html$Attributes.value(model.text)
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (_p2) {    return A2($Signal.message,address,UpdateField(_p2));})
                      ,A2(onEnter,address,Submit(model.text))]),
              _U.list([]))]));
   });
   var Model = F2(function (a,b) {    return {text: a,thoughs: b};});
   return _elm.Counter.values = {_op: _op
                                ,Model: Model
                                ,UpdateField: UpdateField
                                ,Submit: Submit
                                ,update: update
                                ,thoughItem: thoughItem
                                ,thoughList: thoughList
                                ,view: view
                                ,isEnter: isEnter
                                ,onEnter: onEnter};
};
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: {text: "",thoughs: _U.list([])},update: $Counter.update,view: $Counter.view});
